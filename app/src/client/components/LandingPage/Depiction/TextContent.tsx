...
const TextContent: React.FC<TextContentProps> = ({ activeIndex, setSectionsRef }) => {
  ...
  const textContent = {
    desktop: {
      text0: "The formula for the area of a circle, A=πr², is fundamental in geometry. Here, r represents the radius of the circle, and π (approximately 3.14159) is a constant that relates the circle's circumference to its diameter.",
      text1: "This formula works because it integrates the radius into the calculation to define the space enclosed within a circle. For example, a circle with a radius of 3 cm would have an area of about 28.27 square centimeters. Understanding this allows for practical applications, like calculating the surface area of circular objects or spaces.",
      text2: "Interestingly, the concept of π has fascinated mathematicians for centuries, as it is an irrational number that continues infinitely without repeating patterns. Ancient civilizations, like the Egyptians and Babylonians, approximated π long before modern calculations refined its value."
    },
    tablet: {
      text0: "The formula A=πr² is fundamental in geometry. Here, r is the radius, and π (≈3.14159) relates circumference to diameter.",
      text1: "This formula integrates the radius to define the circle's area. A circle with radius 3 cm has area ≈28.27 cm². This helps calculate surface areas of circular objects.",
      text2: "π has fascinated mathematicians as an infinite, non-repeating number. Ancient Egyptians and Babylonians first approximated it."
    },
    mobile: {
    ...
    <div className="relative w-full">
      ...
      >
        <div className="max-w-6xl w-full relative">
          ...
          <div className={`p-4 md:p-8 lg:p-10 rounded-xl md:rounded-2xl transition-all duration-200 ${
            activeIndex === 0 
              ? 'bg-white shadow-lg border-3 border-primary-200' 
              : 'bg-gray-50 border-2 border-gray-100'
          }`}>
            <p className={`text-lg md:text-xl lg:text-2xl ${activeIndex === 0 ? 'font-dancing' : 'font-manrope'} leading-relaxed text-danger min-h-[100px] md:min-h-[150px] lg:min-h-[200px]`}>
              {activeIndex === 0 ? (
                fullText0.split('').map((char, index) => (
                  <span 
                    key={index}
                    className={`transition-opacity duration-100 ${displayedChars0[index] ? 'opacity-100' : 'opacity-0'}`}
                  >
                    {char}
                  </span>
                ))
          ...
          <div className="absolute -top-20 md:-top-24 lg:-top-32 left-1/2 transform -translate-x-1/2">
            <div className="group relative flex items-center gap-3 md:gap-4 lg:gap-6 px-4 md:px-6 lg:px-8 py-2 md:py-3 lg:py-4 rounded-xl md:rounded-2xl bg-white border-2 border-primary-100 transition-all duration-200 hover:border-primary-300">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm md:text-base lg:text-lg font-manrope text-gray-600">Easy but not intuitive</span>
              </div>
            </div>
          </div>
          <div className={`p-4 md:p-8 lg:p-10 rounded-xl md:rounded-2xl transition-all duration-200 ${
            ...
            <p className="text-lg md:text-xl lg:text-2xl font-manrope leading-relaxed text-primary-800 min-h-[100px] md:min-h-[150px] lg:min-h-[200px]">
              {activeIndex === 1 ? (
                fullText1.split('').map((char, index) => (
                  <span 
                    key={index}
                    className={`transition-opacity duration-100 ${displayedChars1[index] ? 'opacity-100' : 'opacity-0'}`}
                  >
                    {char}
                  </span>
                ))
        ...
        <div className="max-w-6xl w-full relative">
          <div className="absolute -top-20 md:-top-24 lg:-top-32 left-1/2 transform -translate-x-1/2">
            <div className="group relative flex items-center gap-3 md:gap-4 lg:gap-6 px-4 md:px-6 lg:px-8 py-2 md:py-3 lg:py-4 rounded-xl md:rounded-2xl bg-white border-2 border-secondary-100 transition-all duration-200 hover:border-primary-300">
              ...
              <div className="flex flex-col">
                <span className="font-satoshi font-bold text-lg md:text-xl lg:text-2xl text-secondary-600">See & Listen</span>
