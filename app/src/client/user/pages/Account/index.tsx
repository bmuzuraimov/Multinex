...
import { useQuery, useAction, updatePrompt, getPrompt, updateUser } from 'wasp/client/operations';
                ...
                <div className='space-y-6'>
                  <div className='flex items-center justify-between'>
                    ...
                    <Switch
                      ...
                      onChange={() => updateUserAction({ send_email: !user.send_email })}
                      className={cn(
...
function PromptCustomizationCard({ user }: { user: User }) {
  ...
  const updatePromptAction = useAction(updatePrompt);
  useEffect(() => {
    if (promptData) {
      setPrePrompt(promptData.data.pre_prompt || '');
      setPostPrompt(promptData.data.post_prompt || '');
    }
  }, [promptData]);
...
export default DefaultLayout(Account);
