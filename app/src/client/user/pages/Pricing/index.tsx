...
import { useAction, stripePayment } from 'wasp/client/operations';
...
const Pricing = () => {
  ...
  const stripePaymentAction = useAction(stripePayment);
  async function handlePaymentClick(tier_id: string) {
    ...
    try {
      ...
      const stripe_results = await stripePaymentAction(tier_id);
...
export default DefaultLayout(Pricing);
