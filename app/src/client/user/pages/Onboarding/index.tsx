...
import { useAction, updateUser, createOnboarding } from 'wasp/client/operations';
...
const Onboarding: React.FC = () => {
  ...
  const createOnboardingAction = useAction(createOnboarding);
  ...
  const handleOnboardingSubmit = async () => {
    ...
    try {
      ...
      await updateUserAction({ onboarding_complete: true });
      navigate('/portal');
    } catch (error) {
        ...
        <Card className='border-none shadow-xl bg-white/80 backdrop-blur-sm'>
          <CardHeader className='space-y-6'>
            ...
            <div className='flex justify-between items-center'>
              ...
              <Button variant='ghost' onClick={handleSkip} className='text-primary-600 hover:text-primary-700'>
                Skip →
              </Button>
            </div>
                    ...
                    {Object.entries(subject_interests).map(([key, value]) => (
                      <Label
                        ...
                        className={cn(
                          'flex items-center p-4 rounded-xl transition-all duration-300 cursor-pointer',
                          'border-2',
                          value ? 'border-primary-500 bg-primary-50' : 'border-primary-100 hover:border-primary-200'
                        )}
                      >
                        <Checkbox
                          checked={value}
...
export default DefaultLayout(Onboarding);
